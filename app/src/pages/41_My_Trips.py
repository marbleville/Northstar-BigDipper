...
tab1, tab2, tab3 = st.tabs(
    ["View Trips & Itinerary", "Create New Trip", "Update Itinerary"])
        ...
        if r.status_code == 200:
            ...
            if trips:
                ...
                selected_label = st.selectbox(
                    "Select a trip to view its itinerary:", list(trip_options.keys()))
                ...
                filtered_trips = [
                    {k: t.get(k) for k in display_cols} for t in trips]
                ...
                if r2.status_code == 200:
                    ...
                    if itinerary:
                        ...
                        filtered_itin = [
                            {k: item.get(k) for k in display_itin} for item in itinerary]
...
with tab2:
    ...
    with st.form("create_trip_form"):
        ...
        trip_type = st.selectbox(
            "Trip Type", ["Leisure", "Business", "Adventure", "Cultural", "Other"])
        ...
        if not destination or not trip_name:
            ...
            try:
                ...
                if r.status_code in (200, 201):
                    st.success(
                        f"Trip '{trip_name}' to {destination} created successfully!")
                    st.rerun()
                else:
                    st.error(
                        f"Failed to create trip. ({r.status_code}) — {r.text}")
            except Exception as e:
                st.error(f"Error: {e}")
...
with tab3:
    ...
    st.write(
        "Load a trip's itinerary, then enter the booking IDs in your preferred order.")

    trip_id_input = st.number_input(
        "Trip ID:", min_value=1, step=1, key="upd_trip_id")
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                if items:
                    ...
                    cols = ['booking_id', 'booking_date',
                            'service_category', 'description', 'booking_status']
                    st.dataframe([{k: i.get(k) for k in cols}
    ...
    booking_ids_input = st.text_input(
        "Enter Booking IDs in new order (comma-separated):", placeholder="e.g. 3, 1, 2")
    ...
    if st.button("Save Order", type='primary'):
        if not booking_ids_input:
            ...
            try:
                id_list = [int(x.strip())
                           for x in booking_ids_input.split(",")]
                payload = {"items": [{"booking_id": bid, "sequence": i+1}
                                     for i, bid in enumerate(id_list)]}
                r = requests.put(
                    f"{API_BASE}/trips/{trip_id_input}/itinerary", json=payload)
                if r.status_code == 200:
